...
exports.findIssues = async (includeBacklog) => {
  const labels = (await settings.get('filterLabels')) || [];
...
exports.updateIssue = async (issue) => {
  ...
  // const { values: [{ id: sprintId }] } = await this.client.getAllSprints(388, 0, 50, 'active');

  // await this.client.addIssueToSprint({ issues: [issue.id] }, sprintId);
