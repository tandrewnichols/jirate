...
exports.forSettings = async () => {
  return await inquirer.prompt([{
    ...
    type: 'input',
    default: [],
    transformer: settings.formatters.filterLabels
  }]);
};

exports.forSetting = async () => {
  return await inquirer.prompt([{
    type: 'list',
    name: 'setting',
    message: 'Which setting do you want to change?',
    choices: Object.keys(settings.knownSettings)
  }]);
};

exports.forValue = async (setting) => {
  const values = settings.knownSettings[setting];
  return await inquirer.prompt([{
    type: values ? 'expand' : 'input',
    name: 'value',
    message: 'Change setting to?',
    ...(values ? {
      choices: values.map((value) => {
        return {
          value,
          name: value.toString(),
          key: value.toString().charAt(0)
        };
      })
    } : {})
  }]);
...
exports.chooseIssue = async (issues) => {
  const options = issues.reduce((memo, issueSet) => {
    if (!issueSet.length) {
      return memo;
    }
